  ...
  wss.on('connection', async (twilioWS: WebSocket) => {
    ...
    function flushInput() {
      ...
      openaiWS.send(JSON.stringify({
        ...
        response: { modalities: ['text', 'audio'], instructions: 'You are a helpful receptionist. Use tools when appropriate. Always call getPolicy at the start of the call and periodically. When using answerQuestion, if top source confidence is below the kbMinConfidence, follow lowConfidenceAction: ask for clarification; or if set to transfer, call transferToHuman; or if voicemail, ask to leave a message. Include a brief spoken citation like: "Source: <top source>" at the end of grounded answers.' }
      }));
    }
    ...
    openaiWS.on('open', () => {
      const sessionUpdate = {
        ...
        session: {
          ...
          tools: [
            { name: 'getPolicy', description: 'Get runtime policy like KB confidence threshold and fallback action', input_schema: { type: 'object', properties: {} } },
            { name: 'setLanguage', description: 'Set conversation language code (e.g., en, es, fr, de)', input_schema: { type: 'object', properties: { lang: { type: 'string' } }, required: ['lang'] } },
            ...
            { name: 'answerQuestion', description: 'Answer caller questions using knowledge base retrieval', input_schema: { type: 'object', properties: { question: { type: 'string' }, language: { type: 'string' } }, required: ['question'] } }
          ]
        }
      };
      ...
      try {
        ...
        if (evt.type === 'response.function_call' || evt.type === 'tool.call') {
          ...
          try {
            ...
            if (name === 'setLanguage') {
              const lang = (args?.lang || 'en').toLowerCase().slice(0,5);
              const voiceMap: Record<string,string> = {
                en: process.env.OPENAI_TTS_VOICE || 'alloy',
                es: 'alloy', fr: 'alloy', de: 'alloy', it: 'alloy', pt: 'alloy'
              };
              const newVoice = voiceMap[lang] || (process.env.OPENAI_TTS_VOICE || 'alloy');
              // Update voice/language mid-call
              openaiWS.send(JSON.stringify({
                type: 'session.update',
                session: {
                  voice: newVoice
              ...
              result = { ok: true, lang, voice: newVoice };
            } else 
            } else if (name === 'bookAppointment' || name === 'takeMessage' || name === 'getBusinessHours' || name === 'answerQuestion') {
          ...
          try {
            const call = currentCallSid ? await prisma.callLog.findUnique({ where: { callSid: currentCallSid } }) : null;
            await prisma.intentLog.create({ data: { intent: String(name), meta: JSON.stringify({ args, callSid: currentCallSid }), callLogId: call?.id } });
          } catch {}
        }
      } catch (e) {
