...
### KB Auto-Indexing & Spoken Citations
- Adding a KB article in the Admin UI auto-indexes it for retrieval (embeddings).
- When KB answers are used, the assistant appends a brief spoken citation (“Source: …”).

### KB Accuracy & Deflection Dashboard
- New: `/admin/analytics-pro?token=...` shows **deflection rate** for calls using KB and per-intent volumes.
- Tip: Ensure your Twilio **Status Callback** is set to `POST /voice/status` so outcomes are tracked.

### Background Jobs (BullMQ)
- Requires `REDIS_URL` (already used). Start workers: `npm run worker`.
- KB indexing and transcription run as background jobs for scale and resilience.

### Per-source Confidence & Citations UI
- `answerQuestion` now logs **per-source confidence** (normalized) to `CitationsLog`.
- Admin → **Citations** shows recent KB answers with source titles and confidence %.

### Voicemail Transcription (OpenAI STT)
- Worker fetches Twilio recordings (uses your `TWILIO_ACCOUNT_SID` and `TWILIO_AUTH_TOKEN` for Basic Auth) and posts to OpenAI **audio transcriptions** (`OPENAI_STT_MODEL`, default `whisper-1`).
- Start workers with `npm run worker`. Ensure `REDIS_URL` is set.

### KB Confidence Thresholds
- Edit in Admin → Hours: **KB Min Confidence** and **Low Confidence Action** (ask for clarification / transfer / voicemail).
- The AI calls `getPolicy()` and adapts behavior accordingly.

### Jobs Admin
- Admin → Jobs shows counts for KB indexing and transcription queues, plus recent failures.
