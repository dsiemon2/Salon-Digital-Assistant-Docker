...
router.post('/admin/hours', requireToken, async (req, res) => {
  const { address, hoursJson, kbMinConfidence, lowConfidenceAction } = req.body;
  let hours = hoursJson;
  ...
  if (existing) {
    await prisma.businessConfig.update({ where: { id: existing.id }, data: { address, hoursJson: hours, kbMinConfidence: Number(kbMinConfidence || 0.55), lowConfidenceAction: String(lowConfidenceAction || 'ask_clarify') } });
  } else {
    await prisma.businessConfig.create({ data: { address, hoursJson: hours, kbMinConfidence: Number(kbMinConfidence || 0.55), lowConfidenceAction: String(lowConfidenceAction || 'ask_clarify') } });
  }
...
router.get('/admin/citations', requireToken, async (_req, res) => {
  const rows = await prisma.citationsLog.findMany({ orderBy: { createdAt: 'desc' }, take: 200 });
  const parsed = rows.map(r => ({ ...r, sources: (()=>{ try { return JSON.parse(r.sources); } catch { return []; } })() }));
  res.render('admin/citations', { token: res.locals.token, rows: parsed });
});
