...
router.post('/voice/voicemail', async (req, res) => {
  ...
  try { await enqueueTranscription({ recordingUrl: req.body?.RecordingUrl, callSid: req.body?.CallSid }); } catch {}
  const twiml = new VoiceResponse();
...
router.post('/voice/status', async (req, res) => {
  const { CallSid, From, To, CallStatus, Timestamp } = req.body || {};
  // upsert call log
  let call = await prisma.callLog.findUnique({ where: { callSid: CallSid || '' } });
  if (!call) {
    try {
